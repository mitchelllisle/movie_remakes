...
allMovies <- read.csv("data/allMovies_new.csv", stringsAsFactors = FALSE)
movieMetaData <- read.csv("data/movieData.csv", stringsAsFactors = FALSE)
...
Which remakes were the most popular? There are a number of movies that donn't hace ratings on IMDB, but looking at the ones that do, `r mostPopularRemakes_data[1,]$Title` is the most popular remake with a rating of `r mostPopularRemakes_data[1,]$imdbrating`
```{r warning=FALSE}
tableWithBar(data = mostPopularRemakes_data, colNames = c("Movie", "IMDB Rating", "Released"), range = mostPopularRemakes_data$imdbrating, pageLength = 20, barColour = '#F4465B')
```
